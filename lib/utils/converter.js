...
function escapeTopicBackLabel(text) {
...
function prependTopicBackLink(markdown, { topicBackLink = '', topicBackLabel = '' } = {}) {
  if (!topicBackLink) {
    return markdown;
  }

  const escapedLabel = escapeTopicBackLabel(topicBackLabel);
  const backLine = `◂ [Back to &lt;${escapedLabel}&gt;](${topicBackLink})`;
  return `${backLine}\n\n${markdown}`;
}
...
export function convertMarkdownToHtml(markdown, { pageTitle = '', topicBackLink = '', topicBackLabel = '' } = {}) {
  try {
    ...
    const htmlBody = marked.parse(prependTopicBackLink(stripped, { topicBackLink, topicBackLabel }));
